...
# Exploring the Data
Now that we have a Table with all the useful information about all the accidents that happened and have it organized, we can begin to explore the accidents in Massachusetts.
...
When looking for the most dangerous places to work in Massachusetts, one may begin to wonder what kind of Occupations could be the most accident prone. 

For instance, below is the Top 10 Most Accident-Prone Occupations from the data.
...
We can also consider what are the results of theses most accident-prone occupation when there is an accident. 
```{r echo=FALSE,fig.width=10}
f + geom_bar() + theme(axis.text.x=element_text(angle=90,hjust=.5,vjust=0.5)) + 
...
We can use the data to consider where in time these accidents happened. There are fluxuations in the frequency 
```{r echo=FALSE,fid.width=10}
...
Visualize where in the state this accidents happened from the Top 10 most Accident-Prone Occupations.
```{r echo=FALSE,fid.width=10}
...
Frequency of Companies in the data can be used to see how many accidents (of all magnitude) can be expected for some timeframe. For instance:
...
This does not look so good so consider when the number of accidents is over two in the decade.
